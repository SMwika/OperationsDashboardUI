...
export const handleApiResponse = (response: IResponse, fullResponse?: boolean) => {
  if (response.error) {
...
export const generateSitesInfoPayload = (filtersState: IFilterState, systemTypes: number[]) => {
  return {
...
export const generateFiltersPayload = (filtersState: IFilterState) => {
  return {
    filter: Object.keys(filtersState).reduce((acc, item) => {
      acc[transformKeys(item)] = Object.values(filtersState[item] || {}) as (string | number)[];
      return acc;
    }, {} as Record<string, (string | number)[]>)
  }
}

export const generatePayload = (filtersState: IFilterState, systemTypes: number[]) => {
  return Object.keys(filtersState).reduce((acc, item) => {
    acc[item] = Object.values(filtersState[item] || {}) as (string | number)[];
    return acc;
...
export const getProgressClassname = (value: number): 'green' | 'yellow' | 'red' => {
  if (value >= 70) {
    return 'green';
  } else if (value >= 40) {
    return 'yellow';
...
export const doRedirect = ({ key, Id, model, filtersState, dispatch, systemTypes } : {key?: string; Id?: number; model: string, filtersState: IFilterState, dispatch: AppDispatch, systemTypes?: number[]}) => {
  const payload = {
    model,
    ...(systemTypes ? generateSitesInfoPayload(filtersState, systemTypes) : generateFiltersPayload(filtersState)),
