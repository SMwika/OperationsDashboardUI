...
const Header: React.FC = () => {
  const { data } = useGetLookupQuery({
    username: sessionStorage.getItem("username"),
  });
  const [anchorLogout, setAnchorLogout] = useState<null | HTMLElement>(null);
  const [anchorOtherApps, setAnchorOtherApps] = useState<null | HTMLElement>(
    null,
  );
  ...
  return (
    <header className='header'>
      <div className='section left'>
        <div
          className='otherApps'
          onClick={(e) => setAnchorOtherApps(e.currentTarget)}>
          <OtherAppsIcon />
        </div>
        <Menu
          className='menu'
          ...
          {data?.menu_list?.map(
            (item: { name_ar: string; name: string; href: string }) => (
              <MenuItem key={item.name}>
                <a href={item.href}>
                  {currentLang === "ar" ? item.name_ar : item.name}
                </a>
              </MenuItem>
            ),
          )}
      ...
      <div className='section middle'>
        {/* <nav>
          ...
          <NavLink to='/unified-system-code'>
            {t("Unified System Code")}
          </NavLink>
          {data?.header_menu_list.map(
            (item: { href: string; name: string }, i: number) => (
              <a key={i} className='nav-link' href={item.href}>
                {t(item.name)}
              </a>
            ),
          )}
